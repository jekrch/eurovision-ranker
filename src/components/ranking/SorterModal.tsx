    ...
    content = (
      <div className="flex flex-col justify-center items-center gap-2 mb-2">
        <div onClick={() => handleChoice('left')} className={classNames("w-full cursor-pointer transition-colors duration-200 md:hover:ring-2 md:hover:ring-blue-400 active:ring-2 active:ring-blue-400 rounded-lg", { "pointer-events-none opacity-75": isComputing })}>
          <SorterContestantCard countryContestant={currentComparison.leftItem} showAsPreviousChoice={previousChoiceForThisStep === 'left'} />
        </div>
        ...
        <div onClick={() => handleChoice('right')} className={classNames("w-full cursor-pointer transition-colors duration-200 md:hover:ring-2 md:hover:ring-blue-400 active:ring-2 active:ring-blue-400 rounded-lg", { "pointer-events-none opacity-75": isComputing })}>
          <SorterContestantCard countryContestant={currentComparison.rightItem} showAsPreviousChoice={previousChoiceForThisStep === 'right'} />
        </div>
      </div>
    );
        ...
        <div className="flex-shrink-0 mt-2 px-4 pb-2">
          {currentSortState?.isComplete ? (
            <div className="flex justify-center items-center space-x-4">
              ...
              {canGoForward && <IconButton onClick={handleForward} disabled={!canGoForward || isComputing} className={classNames("px-4 py-2 text-sm text-white rounded", (!canGoForward || isComputing) ? "bg-gray-700 text-gray-500 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700")} title="Forward" icon={faChevronRight} /> }
            </div>
          ) : (
            isSorterActive && currentSortState && currentComparison && (
              <div className="flex justify-between items-center mt-1">
                ...
                <div className="w-1/3 flex justify-center">
                  <IconButton onClick={onClose} disabled={isComputing} className="px-4 py-2 pr-[1.2em] text-sm text-white bg-gray-500 rounded hover:bg-gray-600 disabled:bg-gray-700 disabled:text-gray-500" title="Cancel" icon={faTimes} />
                </div>
