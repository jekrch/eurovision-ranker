...
export async function getVotes(
  ...
  if (cachedVoteRound !== round || cachedVoteYear !== year) {
    cachedVotes = await fetchVotesForYear(year, undefined, round);
    cachedVoteRound = round;
    cachedVoteYear = year;
  }

  return cachedVotes.filter((v) => !fromCountryKey || v.fromCountryKey == fromCountryKey);
}
...
export async function sortByVotes(
  ...
  votedContestants.sort(
    (a, b) =>
      getContestantVoteFieldValue(b.contestant?.votes, voteTypeFieldName) -
...
async function assignVotesByContestant(
  ...
  const years = new Set(
    countryContestants.filter((cc) => cc.contestant?.year).map((cc) => cc.contestant!.year),
  );
...
export function voteCodeHasSourceCountry(voteCode: string) {
  ...
  const voteCodeParts = voteCode.split('-');
...
export function updateVoteTypeCode(
  ...
  const newTypeString = voteTypes.join('.');
...
function getContestantVoteFieldValue(votes?: ContestantVotes, fieldName?: string): number {
  if (!votes) {
    return 0;
  }
  const value = votes[fieldName as keyof ContestantVotes] as string;
...
export async function assignVotesByContestants(
  ...
  const { fromCountryKey, round } = getVoteCodeSettings(voteCode);

  // if (!fromCountryKey?.length && round == 'final') {
  //     return countryContestants;
  // }

  return await assignVotesByContestant(countryContestants, round, fromCountryKey);
}

function getVoteCodeSettings(voteCode: string) {
  const codes = voteCode?.split('-');

  const roundCode = codes?.[0];
  const fromCountryKey = codes?.[2];
...
export async function fetchVotesByCode(voteCode: string, year: string, toCountryKey?: string) {
  ...
  const votes: Vote[] = await fetchVotesForYear(year, fromCountryKey, round, toCountryKey);
  return votes;
}
