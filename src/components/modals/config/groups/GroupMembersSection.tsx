    ...
    {detail.members && (
      <ul className="space-y-1">
        {detail.members.map((m) => (
          ...
          >
            <div className="w-7 h-7 rounded-full bg-[var(--er-button-neutral)]/40 flex items-center justify-center text-micro text-[var(--er-text-primary)] font-semibold shrink-0">
              {(m.email || '?').charAt(0).toUpperCase()}
            </div>
            <span className="truncate flex-1 text-[var(--er-text-primary)]">{m.email}</span>
            {m.role === 'owner' && (
              <FontAwesomeIcon icon={faCrown} className="text-micro text-amber-400" title="Owner" />
            )}
            {user?.id === m.user_id && (
              <span className="text-micro text-[var(--er-text-subtle)] uppercase">You</span>
            )}
          </li>
        ))}
      </ul>
