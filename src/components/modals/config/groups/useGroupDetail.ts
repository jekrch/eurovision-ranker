...
export function useGroupDetail(
  ...
  const openById = (rankingId: string) => {
    window.location.assign(`${window.location.pathname}?id=${encodeURIComponent(rankingId)}`);
  };
  ...
  const handleNewInvite = async () => {
    try {
      ...
      const link =
        inv.url ??
        `${window.location.origin}${window.location.pathname}?join=${encodeURIComponent(inv.token)}`;
      try {
