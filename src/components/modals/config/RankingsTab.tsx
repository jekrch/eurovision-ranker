...
import {
  ...
  getContestantsByCountry,
} from '../../../utilities/ContestantRepository';
...
const RankingsTab: React.FC = () => {
  ...
  const openAllContestantsByCountry = async (onlyFinalists: boolean = false) => {
    ...
    switch (contestantCountryOrder) {
      case 'Year':
        contestants = contestants.sort((cc1, cc2) => Number(cc2.year) - Number(cc1.year));
        break;
      case 'Rank':
  ...
  return (
    <div className="text-sm space-y-4">
      <p className="text-xs leading-relaxed text-[var(--er-text-tertiary)]">
        Generate official rankings for a contest year, or browse every contestant a country has
        sent.
      </p>
