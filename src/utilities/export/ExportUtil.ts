...
export function convertToCSV(countryContestants: CountryContestant[]): string {
...
export async function convertToJSON(countryContestants: CountryContestant[]): Promise<string> {
  const csv = convertToCSV(countryContestants);
  const jsonArray = await converCsvToJson(csv);
...
export function convertDataToText(countryContestants: CountryContestant[]): string {
...
export const downloadFile = (string: string, fileExtension: string) => {
 ...
 */
export async function copyToClipboard(rankedItems: CountryContestant[], type: EXPORT_TYPE) {
  const data = await getExportDataString(type, rankedItems);
