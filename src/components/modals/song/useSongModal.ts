...
export function useSongModal(props: UseSongModalArgs) {
    ...
    if (year && contestant?.song) {
      ...
      getSongDetails(contestant.id, contestant?.year)
        .then((fetchedSongDetails) => {
          assignLyrics(fetchedSongDetails?.lyrics, fetchedSongDetails?.engLyrics, contestant.song);
      ...
      .then((votes) => {
        ...
        const grouped: RoundVotes[] = Array.from(byRound.entries())
          .map(([round, roundVoteList]) => ({
            round,
            votes: roundVoteList.sort((a, b) => toNumber(b.totalPoints) - toNumber(a.totalPoints)),
          }))
          .sort((a, b) => {
  ...
  const sortedVotes = useMemo(() => {
    ...
    const key = (sortKey === 'jury' || sortKey === 'tele') && !hasJuryTeleSplit ? 'total' : sortKey;
