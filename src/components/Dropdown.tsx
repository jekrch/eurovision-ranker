  ...
  return (
    <Menu as="div" className={classNames('relative inline-block text-left z-40', className)}>
      <div>
        ...
        >
          <span className="truncate min-h-[1.2em] inline-block">{value || '\u00A0'}</span>{' '}
          {/* truncate to handle overflow */}
          {!mini && (
            <FontAwesomeIcon
              className="flex-shrink-0 h-[0.8em] w-4 text-[var(--er-text-subtle)]"
              icon={faChevronDown}
            />
          )}
        </Menu.Button>
                    ...
                    <Menu.Item key={index}>
                      {({ active }) => (
                        <button
                          ...
                          className={classNames(
                            active
                              ? 'bg-[var(--er-surface-light)] text-[var(--er-interactive-text-light)]'
