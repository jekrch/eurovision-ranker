    ...
    if (currentState.currentMergeStep) {
      ...
      if (currentState.mergeStack.length >= 2) {
        ...
        const remainingStack = Object.freeze(
          currentState.mergeStack.filter(
            (_, idx) => idx !== smallestIdx && idx !== secondSmallestIdx,
          ),
        );
...
const withMaxRemainingComparisons = (state: SortState): SortState => {
  ...
  const remaining = calculateMaxRemainingComparisons(state.mergeStack, state.currentMergeStep);
...
export const processChoice = (state: SortState, choice: 'left' | 'right'): SortState => {
  ...
  if (state.isComplete || !state.currentMergeStep || state.action !== ActionType.COMPARE) {
    logger.warn('processChoice called inappropriately.', {
      stateAction: state.action,
      isComplete: state.isComplete,
      hasMergeStep: !!state.currentMergeStep,
    });
    return state;
  }
  ...
  if (!lastComparison || lastComparison.choice) {
    logger.warn(
      `processChoice: Last comparison invalid or already has choice. Index: ${currentComparisonIndex}`,
      lastComparison,
    );
    return state;
  }
...
export const getSortedItems = (state: SortState): CountryContestant[] => {
  if (!state.isComplete || state.action !== ActionType.DONE) {
    logger.warn('getSortedItems called before sorting is complete or in unexpected state.', {
      isComplete: state.isComplete,
      action: state.action,
    });
    return [];
  ...
  if (!state.currentRanking || state.currentRanking.length !== originalValidCount) {
    logger.error(
      `getSortedItems: final ranking count (${state.currentRanking?.length ?? 'undefined'}) mismatch vs original valid items (${originalValidCount}). Returning original items as fallback.`,
    );
